...
for k = 1: numFiles
    ...
    [ndvi,ndre,rdvi,cvi,ci_green,ci_re]  = getVegetativeIndices(masked_Image(:,:,4),masked_Image(:,:,5),masked_Image(:,:,6),masked_Image(:,:,7));
    %imagesc(ndvi)
    %Normalized Difference Vegetation Index (NDVI) = ndvi
    % Normalized Difference Red Edge (NDRE) = ndre
    % %Renormalized Difference Vegetation Index (RDVI) = rdvi
    % %Chlorophyll Vegetation Index (CVI) = cvi
    % %Chlorophyll Index Green (CIg)  = ci_green
    % %Chlorophyll Index Red-edge (CIre)  = ci_re
